...
### Decision 8 — 4.4b direction-alignment check is TRIVIAL/voided; use held-out NLL; move mechanism to CARLA
**Finding (F-35, held-out urban35).** The 4.4b "cov minor-axis vs object-motion direction" check used the **residual direction (ex,ey)** as the object-motion basis. A+ aligned at 11.5° — but the **geometry-only (10-dim, NO motion input) control also aligned at 10.2°**. So alignment is NOT evidence that A+ learned motion: the residual `e = velocity − wheel_predict` is correlated with the hand-prior's anisotropy axis (both derive from the wheel-motion Jacobian in `S`), so "cov-axis ∥ residual" is a **trivial consequence of the hand-prior geometry**, inherited by geom-only. The 4.4b direction check has **no discriminative power and is voided**. (Caught only by running the geom-only control — without it, 11.5° would have been a false green light. Pre-registered prediction "geom-only won't align" was wrong, for this reason.)
**Replacement discriminator — held-out NLL (LOSO, zero-leakage, proper scoring):** A+ −7.537 vs geom-only −7.194 on held-out urban35 → **A+ better by 0.34 nats** (larger than M1a anisotropy +0.16 / vg +0.28). This IS real value from the A+ motion inputs; its mechanism is NOT "direction alignment" (trivial) but something else (magnitude calibration / fixing where the hand-prior is wrong). Per-fold acceptance uses held-out NLL (A+ vs geom-only), not direction alignment.
**Clean mechanism validation moves to CARLA (PRE-TASK 2):** KAIST has no object-motion GT, so the residual-direction basis is a hand-prior-contaminated proxy. The clean "does cov orient anisotropy to object motion" test needs CARLA GT object velocity (PRE-TASK 2), as already planned. **Decisive evidence remains the ATE win gate** (6.2); NLL +0.34 is only an early green light to proceed, not the endpoint (the C2 "good NLL ≠ ATE" lesson is exactly why the ATE gate exists downstream).

### Decision 9 — wheel-on run-to-run nondeterminism: C2 (wheel preload) + V2 investigation; conclusion = NOT eliminable, rely on multi-rep median (spec-evolution record, 2026-06-23)
**Why this section exists:** task 1.4 (re-measure the wheel-on baseline) surfaced that **wheel-on cov-OFF VIO is NOT run-to-run bit-identical** — the SAME binary gives different trajectory md5s run-to-run (urban28 ATE spread ~16–19 m, discrete recurring attractors; ~3–4% of median). This work (C2, V2, the determinism probe) is recorded here because it is **not in the original task list** and a future reader must understand why the C2/preload code and the `det_check_*/repro_*/v1_wheel_*/covon_n1` scripts exist.
- **Diagnosed source(s).** (i) The `/wheel/delta` async callback race: in `WHEEL_REFERENCE_ONLY` mode there is **no coverage-wait** (the wait is gated on `WHEEL_FACTOR_ENABLE`, which is 0), so `buildWheelPreint` integrates whatever the callback happened to deliver → timing-dependent. (ii) A **shared-machine load race**: a CARLA Docker container (`carla_server`, ~3 CPU cores) was contending throughout and induces a further image/IMU async-interleaving nondeterminism.
- **C2 (wheel preload) — built, env-gated/opt-in, DORMANT by default.** Player dumps the deterministic per-sample wheel deltas (`dump_wheel_path`, `%.17g`); estimator preloads them (`REL_WHEEL_PRELOAD`) into `wheel_buffer` and **skips** the `/wheel/delta` subscription → removes the callback race. Fail-hard if preload empty; FOG guard errors if a FOG reference is enabled without a FOG preload. **V1 (value layer): PASS** — dumped wheel == published `/wheel/delta`, 197444/197444 bit-identical.
- **Result (the honest finding).** C2 + **no CARLA**: urban28 cov-OFF was **2/3 bit-identical** (rep1==rep2, rep3 differed), **not 3/3**. C2 + CARLA-load: **0/3** (all different, all full). So **bit-identical determinism is NOT achievable** even with the wheel callback removed and CARLA off — a residual nondeterminism source remains. The trio PRE-TASK 0 "deterministic (std ≤0.1%)" was **wheel-OFF**, which does not have this race.
- **V2 (train/deploy wheel-preint consistency) — DEFERRED, NOT run.** The training perfeat `w_dx/dy/dtheta` were built in reference-only mode (no coverage-wait), so they may be race-affected. V2 would regenerate the perfeat under C2 and diff `w_*`; if different, regenerate all 8 seqs + retrain 8 folds. Deferred because the gates this pass run cov-ON on the **original callback path** (cov-OFF baseline AND cov-ON both callback ⇒ consistent comparison, no preload-vs-callback skew), so V2 is not needed unless C2 is adopted.
- **DECISION (user, 2026-06-23).** Stop trying to drive determinism to 0 (unreachable; a sink). **Rely on spec §"Evaluation rigor" (L101): multi-rep + report median + run-to-run spread** — that is the built-in anti-cherry-pick / anti-"33.73 false-win" mechanism, so the ~3–4% noise is *handled by the protocol*, not eliminated. **C2 and V2 are SHELVED** (code kept, env-gated, dormant by default) until/unless the ATE gate proves Route C worth hardening. All gate runs (6.0/6.1/6.2) use the **original callback path**, multi-rep median, current binary. Consequence: **task 1.4's wheel-on baseline must be measured multi-rep (median + spread), not as a single deterministic number.**
- **SUPERSEDED IN PART by Decision 10 (2026-06-24).** Decision 9's "NOT eliminable / 2/3 best" was measured **with a residual contamination source still present** (the C2+no-CARLA probe ran on a shared machine that still had concurrent load). Under *full isolation* (CARLA off **and** no concurrent workload), the clean four reached **3/3 and 2/2 bit-identical** (Decision 10). So "rely on multi-rep median" is retained, but its status changes: from a *make-do for irreducible noise* to a *confirmation check over an already-deterministic baseline*. C2/V2 remain shelved (no longer needed for determinism; isolation suffices).

### Decision 10 — run-to-run nondeterminism root cause = shared-machine resource contention (CARLA + concurrent load); under full isolation VINS is bit-identical (2026-06-24)
**Finding (clean-four wheel-on cov-OFF baseline, gate binary 15a57897, isolated: CARLA off + machine otherwise idle).** Run-to-run results were **bit-identical** (same `vio.tum` md5, ATE equal to 6 d.p.):
- urban28-pankyo: **3/3** bit-identical (ATE 17.555483 ×3, md5 `7e9b6577`) — including the very sequence Decision 9 flagged with a suspected ~16–19 m (~3–4%) spread.
- urban29-pankyo: 2/2 (8.461941 ×2, md5 `365016c9`); urban26-dongtan: 2/2 (11.931483 ×2, md5 `b1a8237a`); urban27-dongtan: 2/2 (43.847749 ×2, md5 `25c6410f`).
- (trio, same basis: urban35/urban31 were already 3/3 bit-identical; only urban36 showed 2.05% — re-attributable to the same contention, see below.)

**Conclusion (root cause).** The earlier wheel-on nondeterminism was **not** intrinsic VINS estimator nondeterminism. Its root cause was **shared-machine resource contention** — a CARLA Docker container (and other concurrent load) competing for GPU/CPU, which perturbed image/IMU async-callback interleaving run-to-run (the mechanism Decision 9 §"Diagnosed source (ii)" already suspected, now confirmed as the *dominant* one). The Decision 9 §"(i) `/wheel/delta` callback race" was **not** the dominant source: these runs used the **original callback path** (no C2 preload) and were still bit-identical once the machine was isolated. **Under full isolation, VINS is deterministic** (same input → same output, bit-for-bit).

**Hard precondition (carries forward to Phase 2).** This determinism holds **only under isolation**. The contamination returns if a contending workload (CARLA, or asking the machine to do other heavy work mid-run) runs concurrently. Therefore **all Phase 2 cov-ON gate runs (6.0/6.1/6.2) MUST also run isolated** — CARLA off, no concurrent load — or the run-to-run noise comes back and the do-no-harm resolution below is void.

**Methodological upgrade.** "multi-rep + median" (Decision 9 / spec L101) stays in the protocol, but is now a *confirmation* over a baseline that is already bit-identical under isolation, not a *workaround for irreducible noise*. This strengthens the win/do-no-harm gates: the comparison floor is exact, so any cov-induced ATE change is signal, not noise.
...
>
> **[UPDATE 2026-06-24 — Ambush #1 (do-no-harm "noise > signal") RESOLVED by Decision 10.]** The risk that clean-sequence run-to-run noise (then thought ~3–4%) would exceed the 1–2% degradation that 6.1 must detect is **gone**: under isolation the clean baseline is **bit-identical (0% noise)**, established by exact md5 match, not by median-over-noise. So 6.1 can keep the original **1–2% reproducible-Δ resolution** — there is no need to weaken it to absorb baseline noise. **Strict condition:** this only holds if Phase 2 cov-ON runs are **also isolated** (CARLA off, no concurrent load — Decision 10 precondition). If isolation is broken during cov-ON, the noise returns and this resolution is void.
