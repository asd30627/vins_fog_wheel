...
OUTBASE="/mnt/sata4t/ivlab3_data/fwvio/results/route_c/det_check_c2_nocarla/$SEQ"
...
[[ -s "$WHEEL_DUMP" ]] || { echo "[C2NC] FATAL: wheel dump missing: $WHEEL_DUMP"; exit 1; }
echo "[C2NC] binary=$(md5sum "$WS/install/vins/lib/vins/vins_node"|cut -d' ' -f1)  preload=$WHEEL_DUMP ($(($(wc -l <"$WHEEL_DUMP")-1)) samples)  pb1.0 fixedext cov-OFF C2 NO-CARLA  $(date -Iseconds)"
...
run_rep(){  # $1=idx $2=rate-tag -> sets RUN_SPAN; returns 0 if full
  ...
  echo "[C2NC] >>> rep${i} (pb${tag}) START $(date +%H:%M:%S)"
  ...
  if [[ $rc -ne 0 || ! -s "$SCRATCH/vins_raw/vio.tum" ]]; then echo "[C2NC] rep${i} RUNNER FAILED rc=$rc"; tail -15 "$OUT/runner.log"; return 2; fi
  ...
  echo "[C2NC] rep${i} (pb${tag}) DONE span=${RUN_SPAN}s rows=${rw} ate=$(ate_of "$OUT/ape_vio.txt") md5=$(md5sum "$OUT/vio.tum"|cut -d' ' -f1) full=${full}"
  echo "[C2NC] rep${i} C2-log: preload='${preload_line##*\] }'  skip=$([[ -n "$skip_line" ]] && echo YES || echo NO)"
  [[ "$full" -eq 1 ]] && return 0 || return 1
}

for i in $(seq 1 $NREP); do
  if ! run_rep "$i" "1p0"; then
    echo "[C2NC] rep${i} truncated/failed at pb1.0 -> retry pb0.5 (rule out CARLA load spike; truncation != C2 failure)"
    if ! run_rep "$i" "0p5"; then echo "[C2NC] rep${i} STILL not full at pb0.5 -> flag + continue (will mark invalid)"; fi
  fi
done

echo "[C2NC] =========== C2 RESULT: 3 full runs urban28 cov-OFF (preload, no /wheel/delta callback) ==========="
declare -a M
allfull=1
for i in $(seq 1 $NREP); do
  ...
  if [[ -s "$T" ]]; then read sp rw < <(sr_of "$T"); m=$(md5sum "$T"|cut -d' ' -f1); a=$(ate_of "$OUTBASE/rep${i}/ape_vio.txt")
    ...
    echo "[C2NC]   rep${i}: md5=$m ate=$a span=${sp}s rows=${rw} full=${full}"; M[$i]="$m"
  else echo "[C2NC]   rep${i}: MISSING"; allfull=0; fi
done
bid=1; for i in $(seq 2 $NREP); do [[ "${M[$i]}" == "${M[1]}" ]] || bid=0; done
if [[ $allfull -eq 1 && $bid -eq 1 ]]; then
  echo "[C2NC]   VERDICT: ALL $NREP runs BIT-IDENTICAL (md5 equal) + all full -> C2 ELIMINATES the race (std=0)."
  echo "[C2NC]   (Stronger than isolated: CARLA was contending for CPU, yet byte-identical.)"
else
  echo "[C2NC]   VERDICT: NOT all bit-identical/full -> DO NOT conclude C2 failed yet; check 'full' column"
  echo "[C2NC]   (truncated=CARLA load spike, rerun; full-but-different=investigate C2). See per-rep above."
fi
echo "[C2NC] done"
