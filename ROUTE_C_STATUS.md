...
## ⚠️ WHEEL-ON NONDETERMINISM / playback-3.0 TRUNCATION (found 2026-06-22, site-d bit-inv) — BLOCKS ATE
During site-d bit-invariance(1), the **cov-OFF** path was found to diverge wildly run-to-run **under non-isolated, playback_rate 3.0** execution. THREE urban28 wheel-on cov-OFF runs (same env): baseline(dd8d1e0) ATE **18.82 m** (19735 rows, full 1973 s), new1(e7e5575) **632 m** (19735 rows, full — yet diverged!), new2(e7e5575) **34.2 m** (9166 rows, **truncated at 916 s** — VINS fell behind real-time at pb 3.0, rosbag ended first). md5 all different; same-binary new1≠new2 ⇒ nondeterminism is NOT binary/site-d (site d edits are unreachable in cov-OFF: all inside `computeFeatureReliabilityAnisoLearned`, not called when REL_ANISO_INFO unset).
- **Root cause (likely):** non-isolated runs + playback_rate 3.0 throttle ⇒ VINS can't keep up ⇒ load-dependent callback interleaving / dropped data ⇒ divergence + truncation. (Connects to the OPEN "wheel-ref-only changes VIO" callback-interleaving candidate.)
- **PRE-TASK 0 determinism (std ≤0.1%) was WHEEL-OFF.** wheel-ON run-to-run determinism in a CLEAN environment was **never measured** — that is exactly **task 1.4** (still `[ ]`).
- **Watershed (urban28 wheel-on 18.82 beats wheel-off 24.89 by 24%) is NOW QUESTIONABLE:** it was a single playback-3.0 run; the 6 m gap is inside the demonstrated 18.8–632 m run-to-run spread. Re-do with the clean protocol before trusting it.
- **MANDATORY isolation protocol for ALL VIO runs (gate ATE included):** playback_rate **1.0** (not 3.0); strict isolation (one VINS at a time, no concurrent build/python/back-to-back); **completion precondition = full sequence (~19735 rows / ~1973 s span), NOT truncated** — if truncated at 1.0, drop to 0.5 and retry until every run completes. A truncated run masquerades as nondeterminism and is worse than no data.
- **STATUS:** running the clean determinism check (same binary, cov-OFF, pb 1.0, isolated, 3× full runs). ⛔ Until wheel-on determinism is cleanly answered, **do NOT touch ONNX / parity / cov-ON / task 6.0 / ATE**. If 3 full isolated runs are bit-identical / ATE spread <0.1% → wheel-on is clean, 632 m was env contamination, redo bit-inv(1) cleanly. If they still diverge → inherent wheel-on nondeterminism = fundamental protocol problem, re-think evaluation before any ATE.

## Still UNVERIFIED / OPEN
- **wheel-on clean-environment determinism (task 1.4) — IN PROGRESS, gates everything below.**
