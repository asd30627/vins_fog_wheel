...
- **8 LOSO folds trained, held-out NLL (Decision 8 — use MEDIAN, robust): A+ beats geom-only 8/8** by Δ −0.34~−0.74 nats. urban36 mean NLL blew up (175) but that is **0.11% outlier features**; **median = −7.128 (normal)** → benign; A+ still wins. urban36 has 4.5× larger residuals (most dynamic) → a few over-confident feats to watch at ATE.
...
- Decisive evidence = ATE win gate. Held-out NLL +0.34 is only an early green light (C2 lesson: good NLL ≠ ATE).
