...
class IntegrationBase
{
  public:
    ...
    void propagate(double _dt, const Eigen::Vector3d &_acc_1, const Eigen::Vector3d &_gyr_1)
    {
        ...
        regularizeCovariance();   // DEBUG-ONLY env-gated; default off (no behavior change)

    }

    // ===== env-gated preintegration-covariance regularization (DIAGNOSTIC CANDIDATE) =====
    // DEBUG_PREINT_COV_REG_MODE = 0/off | 1/sym_floor_after_propagate | 2/diag_min_bias_only | 3/adaptive_floor
    //   sym_floor : symmetrize + floor all eigenvalues to DEBUG_PREINT_COV_EIG_FLOOR (default 1e-10)
    //   diag_min  : floor bias-block (O_BA..O_BG, dims 9-14) diagonal to DEBUG_PREINT_BIAS_COV_FLOOR (1e-8)
    //   adaptive  : symmetrize + floor eigenvalues to max(ABS_FLOOR, REL_FLOOR*max_diag)
    //               (DEBUG_PREINT_COV_ABS_FLOOR 1e-12, DEBUG_PREINT_COV_REL_FLOOR 1e-6)
    // Default (mode 0) leaves covariance untouched -> identical to stock VINS.
    void regularizeCovariance()
    {
        static const std::string MODE = (std::getenv("DEBUG_PREINT_COV_REG_MODE") ?
                                          std::string(std::getenv("DEBUG_PREINT_COV_REG_MODE")) : "0");
        if (MODE == "0" || MODE == "off") return;
        static const double EIG_FLOOR  = (std::getenv("DEBUG_PREINT_COV_EIG_FLOOR") ?
                                          atof(std::getenv("DEBUG_PREINT_COV_EIG_FLOOR")) : 1e-10);
        static const double BIAS_FLOOR = (std::getenv("DEBUG_PREINT_BIAS_COV_FLOOR") ?
                                          atof(std::getenv("DEBUG_PREINT_BIAS_COV_FLOOR")) : 1e-8);
        static const double REL_FLOOR  = (std::getenv("DEBUG_PREINT_COV_REL_FLOOR") ?
                                          atof(std::getenv("DEBUG_PREINT_COV_REL_FLOOR")) : 1e-6);
        static const double ABS_FLOOR  = (std::getenv("DEBUG_PREINT_COV_ABS_FLOOR") ?
                                          atof(std::getenv("DEBUG_PREINT_COV_ABS_FLOOR")) : 1e-12);
        static const bool   DBG_NUM = (std::getenv("DEBUG_IMU_NUMERIC") &&
                                       std::string(std::getenv("DEBUG_IMU_NUMERIC")) == "1");
        static long reg_call = 0; ++reg_call;
        const bool do_print = DBG_NUM && (reg_call <= 3 || reg_call % 1000 == 0);

        double before_min = 0.0, before_max = 0.0, used_floor = 0.0;
        if (do_print) {
            Eigen::SelfAdjointEigenSolver<Eigen::Matrix<double,15,15>> es0(covariance, Eigen::EigenvaluesOnly);
            before_min = es0.eigenvalues().minCoeff(); before_max = es0.eigenvalues().maxCoeff();
        }

        if (MODE == "2" || MODE == "diag_min_bias_only") {
            // floor only bias-block diagonal (dims 9..14); also enforce symmetry cheaply
            covariance = 0.5 * (covariance + covariance.transpose()).eval();
            for (int i = O_BA; i < 15; ++i)
                if (covariance(i, i) < BIAS_FLOOR) covariance(i, i) = BIAS_FLOOR;
            used_floor = BIAS_FLOOR;
        } else {
            // sym + eigenvalue floor (modes 1 and 3)
            Eigen::Matrix<double,15,15> sym = 0.5 * (covariance + covariance.transpose());
            double floor = EIG_FLOOR;
            if (MODE == "3" || MODE == "adaptive_floor") {
                const double max_diag = covariance.diagonal().maxCoeff();
                floor = std::max(ABS_FLOOR, REL_FLOOR * max_diag);
            }
            Eigen::SelfAdjointEigenSolver<Eigen::Matrix<double,15,15>> es(sym);
            Eigen::Matrix<double,15,1> ev = es.eigenvalues();
            for (int i = 0; i < 15; ++i) if (ev(i) < floor) ev(i) = floor;
            covariance = es.eigenvectors() * ev.asDiagonal() * es.eigenvectors().transpose();
            covariance = 0.5 * (covariance + covariance.transpose()).eval();
            used_floor = floor;
        }

        if (do_print) {
            Eigen::SelfAdjointEigenSolver<Eigen::Matrix<double,15,15>> es1(covariance, Eigen::EigenvaluesOnly);
            const double after_min = es1.eigenvalues().minCoeff();
            const double after_max = es1.eigenvalues().maxCoeff();
            printf("[DBG_COV_REG] mode=%s reg_call=%ld sum_dt=%.4f before_min_eig=%.6e after_min_eig=%.6e "
                   "before_cond=%.6e after_cond=%.6e floor=%.3e\n",
                   MODE.c_str(), reg_call, sum_dt, before_min, after_min,
                   (before_min != 0.0 ? before_max / before_min : -1.0),
                   (after_min  > 0.0 ? after_max / after_min : -1.0), used_floor);
            fflush(stdout);
        }
    }
