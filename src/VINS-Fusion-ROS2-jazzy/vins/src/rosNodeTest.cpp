...
void reliability_prediction_callback(const std_msgs::msg::Float64MultiArray::SharedPtr msg)
{
    ...
    // 只傳模型原始 reliability score。
    // 不在 callback 階段做 min-soft-weight clamp。
    // 最終 backend alpha 由 Estimator::applyVisualResidualWeight()
    // 透過 uncertainty mapping + smoothing 決定。
    ...
    if ((cnt++ % 100) == 0)
    {
        ROS_WARN("[admission] received prediction: w=%.3f p_fail=%.3f p_harmful=%.3f",
                w_pred,
                msg->data[3],
                msg->data[4]);
    }
...
int main(int argc, char **argv)
{
    ...
    {
        bool ia_enable = n->get_parameter_or("visual_internal_alpha_schedule_enable", false);
        double ia_value = n->get_parameter_or("visual_internal_alpha_value", 1.0);
        double ia_start = n->get_parameter_or("visual_internal_alpha_start_ts", -1.0);
        double ia_end = n->get_parameter_or("visual_internal_alpha_end_ts", -1.0);
        estimator.setInternalAlphaSchedule(ia_enable, ia_value, ia_start, ia_end);
    }
