...
class Utility
{
  public:
    ...
    static Eigen::Quaterniond expSO3(const Eigen::Vector3d &theta)
    {
        ...
        if (angle < 1e-8)
        {
            const Eigen::Vector3d half_theta = 0.5 * theta;
            q.w() = 1.0 - theta.squaredNorm() / 8.0;
            q.x() = half_theta.x();
            q.y() = half_theta.y();
            q.z() = half_theta.z();
        }
    ...
    static Eigen::Vector3d logSO3(const Eigen::Quaterniond &qin)
    {
        ...
        const double w = std::max(-1.0, std::min(1.0, q.w()));
        const double angle = 2.0 * std::acos(w);
        const double s = std::sqrt(std::max(0.0, 1.0 - w * w));

        if (s < 1e-8)
        {
            return 2.0 * Eigen::Vector3d(q.x(), q.y(), q.z());
        }

        return angle * Eigen::Vector3d(q.x(), q.y(), q.z()) / s;
    }

    static double quatAngleDeg(const Eigen::Quaterniond &qin)
    {
        Eigen::Quaterniond q = qin;
        q.normalize();
