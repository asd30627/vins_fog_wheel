...
class KaistPlayerNode(Node):
    def __init__(self) -> None:
        ...
        # Skip a leading static prefix (seconds from the dataset's first timestamp). Default 0.0 =
        # no trim (backward-compatible). Used by Paper-2 System A stereo-only, where a parked start
        # (no parallax) makes pure-stereo VINS init degenerate and diverge.
        self.declare_parameter('start_offset_sec', 0.0)
        # P1a replay-layer visual outage: blacken stereo images for a time window
        # [start, start+dur) seconds relative to the FIRST published stereo frame. dur<=0 = off.
        # Estimator/IMU/clock untouched; only image CONTENT is zeroed (true visual outage, fair A/B).
        self.declare_parameter('img_blackout_start_sec', -1.0)
        self.declare_parameter('img_blackout_dur_sec', -1.0)
        ...
        self.img_blackout_start_sec = float(self.get_parameter('img_blackout_start_sec').value)
        self.img_blackout_dur_sec = float(self.get_parameter('img_blackout_dur_sec').value)
        self._stereo_t0_ns = None
        ...
        start_offset_sec = float(self.get_parameter('start_offset_sec').value)
        if start_offset_sec > 0.0:
            cutoff_ns = self.events[0].ts_ns + int(start_offset_sec * 1e9)
            n_before = len(self.events)
            self.events = [e for e in self.events if e.ts_ns >= cutoff_ns]
            if not self.events:
                raise RuntimeError(f'start_offset_sec={start_offset_sec} trimmed all events.')
            self.get_logger().info(
                f'start_offset_sec={start_offset_sec}: trimmed {n_before - len(self.events)} '
                f'leading events (new start ts {self.events[0].ts_ns})')
    ...
    def _publish_stereo(self, frame: StereoFrame) -> None:
        ...
        # P1a replay-layer visual outage: zero image content inside [start, start+dur) sec of first stereo frame.
        if self.img_blackout_dur_sec > 0.0:
            if self._stereo_t0_ns is None:
                self._stereo_t0_ns = frame.ts_ns
                self.get_logger().info(
                    f'img_blackout window = [{self.img_blackout_start_sec:.1f}, '
                    f'{self.img_blackout_start_sec + self.img_blackout_dur_sec:.1f}) s rel first stereo'
                )
            rel = (frame.ts_ns - self._stereo_t0_ns) / 1e9
            if self.img_blackout_start_sec <= rel < (self.img_blackout_start_sec + self.img_blackout_dur_sec):
                left_img = np.zeros_like(left_img)
                right_img = np.zeros_like(right_img)
